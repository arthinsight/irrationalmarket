...
function hfBusiness(){let p=D.profile||{},products=productList(),markets=marketList();return'<div class="profile-band"><div><span>Operating entities</span><b>'+E(p.entities)+'</b></div><div><span>Business lines</span><b>'+E(p.subseg)+'</b></div><div><span>Structural themes</span><b>'+E(p.themes)+'</b></div></div><div class="layout-2">'+ddCards('business',['challenges'])+(products.length?card('Products and platforms',list(products,12)):'')+(markets.length?card('End markets',list(markets,12)):'')+drhpOverviewCards()+'</div>'}
...
function explorer(){let sets=P.explorer||{},labels={momentum:'Momentum',investing:'Investing — Both',investing_complete:'Investing — setup complete',investing_building:'Investing — setup building',new:'New listings',peers:'Peers'},
/* `investing` is kept in the key list for payloads written before the split (2026-08-25) — a page
   served from a cached older payload should still show its list rather than an empty picker. Keys
   with no rows are dropped below, so it costs nothing once every payload carries the new sets. */
keys=['momentum','investing','investing_complete','investing_building','new','peers'].filter(k=>A(sets[k]).length||k==='peers');return'<aside class="stock-explorer"><div class="explorer-title"><div><span>Company Explorer</span><b>Stock list</b></div><button id="explorer-close" aria-label="Close stock list">×</button></div><select id="explorer-source" aria-label="Choose stock screen">'+keys.map(k=>'<option value="'+k+'">'+labels[k]+'</option>').join('')+'</select><input id="explorer-search" type="search" placeholder="Filter stocks…" aria-label="Filter stock list"><div id="explorer-list"></div></aside><button id="explorer-open" class="explorer-open">Stocks</button>'}
function initExplorer(){let sets=P.explorer||{},source=document.getElementById('explorer-source'),search=document.getElementById('explorer-search'),listBox=document.getElementById('explorer-list'),aside=document.querySelector('.stock-explorer');if(!source||!search||!listBox||!aside)return;/* A screen's filter row narrows the LIST as well as its own table (owner 2026-08-25: "When filter
   are changed the stock explorer list would be updated"). `allow` is null on an ordinary company
   page and on a screen with no filters applied, in which case nothing here changes. */
let allow=null;
/* KEEP THE READER WHERE THEY WERE. Clicking a stock is a real navigation (href=/company/?sym=),
   so every click reloads the page, re-renders this list and resets scrollTop to 0 - the row just
   clicked is then hundreds of rows down and has to be hunted for again (owner 2026-08-31).
   Centre the selected row INSIDE the list box. Never scrollIntoView(): the list is a nested
   scroller and that also scrolls the DOCUMENT, throwing the reader to the top of the company
   page - trading one wrong scroll position for another. */
function centre(){let sel=listBox.querySelector('.explorer-stock.selected');if(!sel)return;let r=sel.getBoundingClientRect(),b=listBox.getBoundingClientRect();listBox.scrollTop+=(r.top-b.top)-(b.height/2-r.height/2)}
function draw(keep){let q=String(search.value||'').toUpperCase(),rows=A(sets[source.value]).filter(r=>(!allow||allow.has(String(r.symbol||'').toUpperCase()))&&(!q||String(r.symbol||'').toUpperCase().includes(q)||String(r.name||'').toUpperCase().includes(q)));listBox.innerHTML=rows.length?rows.map(r=>'<a class="explorer-stock'+(r.symbol===I.symbol?' selected':'')+'" href="/company/?sym='+encodeURIComponent(r.symbol)+'"><b>'+E(r.symbol)+'</b><span>'+E(r.name||r.meta||'')+'</span><small>'+E(r.meta||'')+'</small></a>').join(''):'<p class="empty">No stocks in this view.</p>';if(keep!==false)centre()}source.onchange=()=>draw();search.oninput=()=>draw(false);document.getElementById('explorer-close').onclick=()=>aside.classList.remove('open');document.getElementById('explorer-open').onclick=()=>aside.classList.add('open');
/* SCREEN MODE: pin the sidebar to this screen's set, and switch symbol IN PLACE.
   The list keeps rendering real <a href> — middle-click, open-in-new-tab and copy-link must keep
   working, and the company page stays reachable at a real URL. The handler only PREVENTS the
   navigation when it can do better, which is exactly when M2D holds a stage record for the symbol
   (`M2D.recOf` — the same question company_page.js already asks to gate its Stage section).
   No record => let the link navigate, because the company page is then the right destination. */
if(SC){
    if(SC.explorer&&A(sets[SC.explorer]).length){source.value=SC.explorer;}
    if(SC.pinExplorer){source.style.display='none';}
    /* The screen's own filter row calls this; passing null clears the narrowing. Exposed rather
       than reading filter state from here, because the filters belong to the screen's table and
       this renderer must not know what a "gate" or a "board" is. */
    window.__EXPLORER_FILTER__=function(symbols){
        allow=symbols&&symbols.length!==undefined?new Set(symbols.map(x=>String(x).toUpperCase())):null;
        draw();
    };
    /* The SWITCH ITSELF belongs to the boot, not here. The first version gated on
       `M2D.recOf(sym)`, but `M2D.init` is called with only the CURRENT company
       (`stocks:M.stock?[M.stock]:[]`), so `recOf` answers null for every other symbol and every
       click fell through to a full navigation — the exact reload this was meant to remove. The boot
       is the thing that knows how to fetch and assemble a company, so it exposes the handler and we
       just ask for it. No handler (an ordinary company page) => the link navigates, as it should. */
    listBox.addEventListener('click',ev=>{
        let a=ev.target.closest&&ev.target.closest('a.explorer-stock');if(!a)return;
        if(ev.metaKey||ev.ctrlKey||ev.shiftKey||ev.button)return;      // let the browser do its thing
        let m=/[?&]sym=([^&#]+)/.exec(a.getAttribute('href')||'');if(!m)return;
        if(typeof window.__SCREEN_SELECT__!=='function')return;
        ev.preventDefault();
        window.__SCREEN_SELECT__(decodeURIComponent(m[1]).toUpperCase());
    });
    addEventListener('popstate',()=>{                                   // Back moves between symbols
        let m=/[?&]sym=([^&#]+)/.exec(location.search||'');
        if(m&&typeof window.__SCREEN_SELECT__==='function')
            window.__SCREEN_SELECT__(decodeURIComponent(m[1]).toUpperCase(),true);
    });
}
draw()}
function initStage(){if(!window.M2D)return;/* `'../data'` is right for /company/ and WRONG for a page any deeper: on /screens/<name>/ every
   lazy per-symbol fetch resolved to /screens/data/... and missed, so the charts stayed empty while
   the rest of the page looked fine. The screen config carries its own depth-correct path. */
window.M2D.init({stocks:M.stock?[M.stock]:[],meta:{hist_sessions:130},datadir:(SC&&SC.datadir)||'../data'});if(M.stock){window.M2D.select(I.symbol);let bs=document.querySelectorAll('#m2-tf button');bs.forEach(b=>b.onclick=()=>{bs.forEach(x=>x.classList.toggle('active',x===b));window.M2D.setTf(+b.dataset.tf)})}else if(A(M.price).length){window.M2D.setCur(I.symbol)}window.M2D.loadDelVol()}
...
/* On a SCREEN the hero names a company you are only previewing, so it carries the way out to
   the full page - the same "Analyze" affordance the IPO calendar puts on every row (owner
   2026-08-25). `SC` is null on /company/ itself, where there is nowhere to go, so it is omitted
   there rather than linking a page to itself. */
/* The Momentum TAB shows an insights card under the company name - one-line thesis, sector /
   layer / cluster chips, "Themes & need of the hour" with tier and tag, and near-term triggers -
   and the screen did not (owner 2026-08-25, quoting NEULAND's). This mirrors that block. It is
   fed by `P.screenInsights`, which the screen boot fills from the momentum payload, because the
   company store's `momentum.stock` is empty for these symbols. Null on ordinary company pages. */
function screenInsights(){
    let I=P.screenInsights;if(!I)return'';
    let chip=v=>A(v).filter(Boolean).map(x=>'<span class="evidence fact">'+E(x)+'</span>').join('');
    let out='';
    /* The one-line thesis and the concall attribution line ("NEULAND LAB LTD. · Q4FY26 · call
       2026-05-16") were both dropped on 2026-08-25 at the owner's request. The thesis repeats what
       the hero already says at the top of the same page, and the attribution named a call the
       reader has no way to open from here. What is left is what this card alone carries: the
       sector / layer / cluster chips and Themes & need of the hour. */
    let tags=chip(I.sec)+chip(I.layer)+chip(I.cluster);
    if(tags)out+='<div class="legend">'+tags+'</div>';
    let st=A(I.st);
    if(st.length){
        let byT={};st.forEach(x=>{(byT[x.theme]=byT[x.theme]||[]).push(x)});
        out+='<p class="src-title">Themes &amp; need of the hour</p>';
        Object.keys(byT).forEach(t=>{
            out+='<p class="src-title">'+E(t)+'</p><ul class="intel-list">';
            byT[t].forEach(x=>{
                let tier=(x.status!=='Unthemed'&&x.tier)?'<span class="evidence guide">T'+E(x.tier)+'</span> ':'';
                out+='<li>'+tier+'<b>'+E(x.subtheme||'')+'</b>'+(x.tag?' '+E(x.tag):'')+(x.status?' &middot; '+E(x.status):'')+'</li>';
            });
            out+='</ul>';
        });
    }
    let trig=A(I.trig);
    if(trig.length){
        out+='<p class="src-title">Near-term triggers</p><ul class="intel-list">'+
            trig.map(t=>'<li>'+(t.im?'<span class="evidence guide">'+E(t.im)+'</span> ':'')+'<b>'+E(t.tg||t.tm||'')+'</b>'+((t.tm&&t.tg)?' &middot; '+E(t.tm):'')+'</li>').join('')+'</ul>';
    }
    // card() escapes its own title, so the ampersand goes in as a plain character, not as markup.
    return out?card('Theme & concall context',out):'';
}
function analyzeLink(){return (SC&&SC.company)?'<a class="screen-analyze" href="'+E(SC.company)+'?sym='+encodeURIComponent(I.symbol)+'">Analyze &#8599;</a>':''}
function hero(){let st=M.stock,sc=P.scorecard||{},an=P.ipo?.analysis||{},hasDrhp=PM?PM.coverage?.drhp:P.coverage.drhp,hasDeep=PM?PM.coverage?.deepDive:P.coverage.deepDive,f=[['Coverage',hasDeep&&hasDrhp?'Deep Dive + offer research':hasDrhp?'Offer-document research':hasDeep?'Operating deep dive':'Market coverage'],['Market structure',st?st.g+' · '+({1:'Basing',2:'Advancing',3:'Top',4:'Decline'}[st.g]||'Tracked'):'Classification pending']],thesis=PM?.hero?.oneLiner||P.summary.oneLiner;if(!PM&&I.symbol==='INDOMIM')thesis=String(thesis||'').replace(/ in Calendar Year 2025 for the last six years/i,', a leadership position held for six years');if(sc.symbol)f.push(['Business quality',sc.bq_total+' / 100']);/* The scorecard has TWO scores and the hero showed one (owner 2026-08-25: "financial score only
   Business quality is showing... we have two on old tab e.g neuland BQ 86 - S 65"). `per_total`
   is the setup score the Investing tab prints as "S" - already in the sidecar, just unread.
   Verified on NEULANDLAB: bq_total 86, per_total 65, matching the tab exactly. */
if(sc.symbol&&sc.per_total!=null&&sc.per_total!=='')f.push(['Setup score',sc.per_total+' / 100']);if(an.Symbol)f.push(['Market cap','₹'+N(an['Market Cap (Cr)'],0)+' cr']);let hasModel=A(P.projection?.projection).length;return'<section class="company-hero"><p class="eyebrow">Institutional company intelligence</p><div class="title-row"><h1>'+E(I.name)+'</h1><span>'+E(I.symbol)+'</span>'+analyzeLink()+'</div><p class="thesis">'+E(thesis)+'</p><div class="decision-strip">'+f.map(x=>'<div><span>'+E(x[0])+'</span><b>'+E(x[1])+'</b></div>').join('')+'</div><div class="legend">'+badge('Reported fact')+badge('Management guidance','guide')+(hasModel?badge('Model estimate','estimate'):'')+badge('Analytical inference','inference')+'</div></section>'}
...
function listing(){let s=P.ipo?.summary||{},a=P.ipo?.analysis||{};return s.Symbol?'<div class="stack">'+offerUseCard()+anchorCard()+'<div class="layout-2">'+card('Price discovery',table(['Issue','Open','Listing gain','Latest','Since open'],[[N(s['Issue Price']),N(s['Listing Open Price']),N(s['Listing Gain %'])+'%',N(a['Latest Close']),N(a['Return From Listing %'])+'%']]))+card('Offer structure',list(sec('objects_execution').objects,6))+card('Forensic checks',list(sec('intellisense').forensic,8))+'</div></div>':(anchorCard()||'<div class="empty">No offer record applies to this coverage.</div>')}
...
/* Board & management, matching the internal DRHP tab. The public page showed only Name and Role,
   discarding qualification (668 people), experience (736) and other directorships (526) that the
   filing states - 773 people across 101 symbols. Age rides with the name and the directorships sit
   under it as a note, so the table stays three columns on a phone.
   THREE rows visible: a board is a long list and the first names are the ones a reader checks.
   table() carries its own show-more, so the rest are one click away rather than truncated. */
function boardTable(people){
    let rows=A(people).filter(x=>x&&(x.name||x.role||x.designation)).map(x=>{
        let dirs=A(x.other_directorships).filter(Boolean);
        let who=E(x.name||'-')+(x.age?' <small class="row-note">age '+E(x.age)+'</small>':'')
              +(dirs.length?'<small class="row-note">Also on: '+E(dirs.slice(0,4).join(', '))
                +(dirs.length>4?' +'+(dirs.length-4)+' more':'')+'</small>':'');
        let bg=[x.qualification,x.experience].filter(Boolean).map(v=>E(String(v))).join(' &middot; ');
        return [who,E(x.role||x.designation||'-'),bg||'-'];
    });
    if(!rows.length)return '';
    return table(['Name','Role','Background'],rows,3);
}
...
/* Growth durability — `intellisense.growth_durability`, 101 symbols. The verdict word carries the
   claim, the signals table carries the working. Every figure is stated; nothing is derived here. */
function growthDurabilityCard(){
    let g=((R.intellisense||{}).growth_durability)||{};
    if(!g||!Object.keys(g).length)return '';
    let v=String(g.verdict||'').trim();
    let facts=[
        {label:'Achievable CAGR',value:g.achievable_cagr_pct==null?'—':N(g.achievable_cagr_pct,1)+'%'},
        {label:'Trailing CAGR',value:g.trailing_cagr_pct==null?'—':N(g.trailing_cagr_pct,1)+'%'},
        {label:'Cohort median',value:g.cohort_median_growth_pct==null?'—':N(g.cohort_median_growth_pct,1)+'%'},
        {label:'Hard reds',value:g.hard_reds==null?'—':N(g.hard_reds,0)},
    ].filter(x=>x.value!=='—');
    let rows=A(g.signals).map(x=>[
        E(String(x.signal||'').replace(/_/g,' ')),
        E(x.status||x.direction||'—'),
        E(x.note||'—'),
    ]);
    let body=(facts.length?kpis(facts):'')
        +(g.headline_reason?'<p>'+E(g.headline_reason)+'</p>':'')
        +(g.achievable_basis?'<p class="method-note">Basis: '+E(g.achievable_basis)+'</p>':'')
        +(rows.length?table(['Signal','Reads','Evidence'],rows,6):'');
    if(!body)return '';
    return card('Growth durability'+(v?' — '+v:''),body,'inference-card');
}
...
// "Where the money goes" — the internal DRHP dashboard has shown this since launch and the public
// page rendered the same data as a flat rupee-crore table, which loses the proportions. Bars carry
// the share of the offer each object takes; the caveat rides along because 95 of 96 symbols have
// one, and it is the line that stops a reader treating "objects" as committed capex.
function offerUseCard(){
    let oe=(R.objects_execution||Q.objects_execution||{});
    let objs=A(oe.objects).map(x=>{
        let n=x&&(x.object||x.name||x.purpose||x.item);
        let a=Number(x&&(x.amount_lakhs!=null?x.amount_lakhs:x.amount));
        return (n&&a)?{n:String(n),a:a}:null;
    }).filter(Boolean);
    if(!objs.length)return '';
    let sum=objs.reduce((t,x)=>t+x.a,0)||1;
    let bars=objs.slice(0,8).map(x=>{
        let pct=x.a/sum*100;
        return '<div class="use-bar"><span class="use-n">'+E(x.n)+'</span>'+
               '<span class="use-track"><span class="use-fill" style="width:'+N(pct,0)+'%"></span></span>'+
               '<span class="use-amt">'+N(x.a,0)+' &middot; '+N(pct,0)+'%</span></div>';
    }).join('');
    let onp=oe.orders_not_placed;
    let note=(onp&&(onp.pct!=null||onp.note))
        ? '<p class="method-note">Caveat: '+E(String(onp.note||('orders not yet placed for '+onp.pct+'% of capex')))+'</p>' : '';
    let more=objs.length>8?'<p class="method-note">'+(objs.length-8)+' further object(s) not shown.</p>':'';
    // card() escapes its title, so the unit goes in the title TEXT, not as markup.
    return card('Where the money goes (₹ lakh)',bars+note+more);
}
function coverageListing(){let o=sec('objects_execution'),c=sec('capital_ownership'),s=P.ipo?.summary||{},objects=o.objects||Q.objects||[],anchors=Q.anchor_allotment||{},uses=table(['Use of proceeds','₹cr'],A(objects).map(x=>[E(x.purpose||value(x)),x.amount_lakhs==null?E(x.amount||'—'):N(x.amount_lakhs/100,0)]));let discovery=s.Symbol?table(['Issue price','Listing open','Listing gain'],[['₹'+N(s['Issue Price']),'₹'+N(s['Listing Open Price']),N(s['Listing Gain %'],1)+'%']]):'';return'<div class="stack">'+offerUseCard()+card('Offer objects',uses)+card('Offer structure',list([c.dilution,c.lock_in,c.pledging].filter(Boolean),8))+card('Anchor allocation',kpis([{label:'Anchor investors',value:anchors.n||'—'},{label:'Allocation',value:anchors.total_amount?'₹'+N(anchors.total_amount/10000000,1)+'cr':'—'}]))+anchorCard()+card('Price discovery',discovery)+'</div>'}
...
/* DRHP-only Business cards, mirroring the internal `boTab`. They read `drhp.rpt` (101 symbols) and
   omit themselves entirely when it is absent, so the 1,700-odd operating companies that share this
   renderer are unaffected. Written as one function called from the renderer rather than pasted into
   each, because a card added to one of several dispatch targets reaches a fraction of the page. */
function drhpOverviewCards(){
    let ov=(R.overview||{}), bo=(R.business_ops||{});
    let out='';
    // "What the Company Does" answers WHAT; this answers HOW IT EARNS. Different question, and the
    // internal dashboard has carried both side by side since launch.
    if(String(ov.business_model||'').trim())
        out+=card('Business model',prose(ov.business_model));
    // Operations & capacity — plants with their utilisation where the DRHP states it.
    let plants=A(bo.plants).map(x=>{
        if(!x||typeof x!=='object')return [E(String(x||'')),'—','—'];
        let loc=String(x.location||x.site||'—').split(/[(]/)[0].trim();
        return [E(loc),E(x.area||'—'),E(x.note||x.capacity||'—')];
    });
    let util=bo.capacity_utilization;
    let utilRows=[];
    if(util&&typeof util==='object'&&!Array.isArray(util))
        utilRows=Object.keys(util).map(k=>[E(k),E(String(util[k]))]);
    else if(Array.isArray(util))
        utilRows=util.map(x=>[E(x&&(x.line||x.product||x.period)||'—'),E(String(x&&(x.pct||x.utilisation||x.value)||'—'))]);
    let ops=(plants.length?table(['Plant','Area','Note'],plants,6):'')
          +(utilRows.length?table(['Capacity utilisation','Stated'],utilRows,6):'')
          +(typeof util==='string'&&util.trim()?prose(util):'');
    if(ops) out+=card('Operations & capacity',ops);
    // Concentration — the DRHP states these as prose notes far more often than as tables, so the
    // note is rendered as written rather than parsed into columns it does not have.
    // The common shape is {top1_pct_by_fy:{FY:pct}, top5_pct_by_fy:{...}, top10_pct_by_fy:{...}}.
    // The first version handled string / .note / array and missed it entirely - AASTHA rendered only
    // because it happened to carry a `note`, which is why this looked like it worked.
    let conc=x=>{
        if(!x)return '';
        if(typeof x==='string')return prose(x);
        if(Array.isArray(x))return table(['Period','Share'],x.map(r=>[E(r&&(r.period||r.fy)||'—'),E(String(r&&(r.pct||r.share)||'—'))]),6);
        let buckets=Object.keys(x).filter(k=>/_pct_by_fy$/.test(k)&&x[k]&&typeof x[k]==='object');
        if(buckets.length){
            let fys=[];
            buckets.forEach(b=>Object.keys(x[b]).forEach(f=>{if(!fys.includes(f))fys.push(f)}));
            let label=b=>'Top '+(b.match(/top(\d+)/)||[,'?'])[1];
            let rows=fys.map(f=>[E(f)].concat(buckets.map(b=>x[b][f]==null?'—':N(x[b][f],1)+'%')));
            let out=table(['Period'].concat(buckets.map(label)),rows,6);
            return out+(x.note?prose(x.note):'');
        }
        if(x.note)return prose(x.note);
        return '';
    };
    let cust=conc(bo.customer_concentration), supp=conc(bo.supplier_concentration);
    if(cust||supp)
        out+=card('Customer & supplier concentration',
                  (cust?'<p class="src-title">Customers</p>'+cust:'')+
                  (supp?'<p class="src-title">Suppliers</p>'+supp:''));
    return out;
}
function opBusiness(){
    ...
    return'<div class="profile-band"><div><span>Operating entities</span><b>'+E(p.entities||'—')+'</b></div><div><span>Business lines</span><b>'+E(p.subseg||'—')+'</b></div><div><span>Structural themes</span><b>'+E(p.themes||'—')+'</b></div></div><div class="stack">'+card('What the Company Does',prose(whatTheyDo.body))+drhpOverviewCards()+moatCard+(cap.body?card('Manufacturing capacity',prose(tidyCapacity(cap.body),8)):'')+(certs.body?card('Technical Moats & Certifications',prose(certs.body)):'')+(products.length?card('Products and platforms',list(products,12)):'')+(markets.length?card('End markets',list(markets,12)):'')+'</div>'
}
...
/* Peers as the DRHP itself states them - `industry_peers.peers_drhp`, 78 symbols. These are the
   issuer's own chosen comparables with the figures it published, which is a different claim from
   our peer panel: the DRHP picks who it wants to be measured against. Kept separate and labelled
   for that reason, never merged into the operating peer set. */
function drhpPeerCard(){
    let rows=A((R.industry_peers||{}).peers_drhp).map(x=>[
        E(x.name||x.symbol||'-'),
        E(x.fy||'-'),
        x.total_income==null?'-':N(x.total_income/100,1),
        x.eps==null?'-':N(x.eps,2),
        x.pe==null?'-':N(x.pe,1)+'x',
        x.ronw==null?'-':N(x.ronw,1)+'%',
    ]);
    if(!rows.length)return '';
    return card('Peers as stated in the DRHP',
        table(['Company','FY','Total income Rs cr','EPS','P/E','RoNW'],rows,8)+
        '<p class="method-note">The issuer’s own comparable set, with the figures as published '+
        'in the offer document. Not our peer panel, and not adjusted.</p>');
}
function opPeers(){
    ...
    return '<div class="stack">'+Object.keys(groups).map(g=>card(g,table(['Company','FY','Revenue ₹cr','Growth','EBITDA margin','PAT ₹cr','PAT growth','P/E','Market cap ₹cr'],groups[g].map(x=>[E(x.name),E(x.fy),N(x.rev,0),x.rev_growth==null?'—':N(x.rev_growth,1)+'%',N(x.ebitda_margin,1)+'%',N(x.pat,0),x.pat_growth==null?'—':N(x.pat_growth,1)+'%',x.pe==null?'—':N(x.pe,1)+'x',N(x.mcap,0)])))).join('')+drhpPeerCard()+'<p class="method-note">'+E(I.symbol)+' is shown separately. Stored operating peer groups remain distinct lenses rather than one blended reference set.</p></div>'
}
...
/* SCREEN MODE (owner 2026-08-25). A /screens/<name>/ page IS this page, driven by a config the
   boot script sets before the renderer loads. `SC` is null on every ordinary company page, so all
   1,710 of them take the identical path they always have — this is additive, never a branch they
   can fall into by accident.
     SC.sections  restrict the vertical nav to these section ids (Momentum and New Listing show
                  Stage analysis only; Investing keeps the full page).
     SC.explorer  which explorer set to pin the sidebar to.
   A screen that names sections nothing on this symbol provides keeps the model's own list rather
   than rendering an empty page - a symbol with no stage record is a data gap, not a blank screen. */
const SC=(typeof window!=='undefined'&&window.__SCREEN__)||null;
if(SC&&A(SC.sections).length){
    let want=A(SC.sections),kept=verticalTabs.filter(t=>want.includes(t[0]));
    if(kept.length)verticalTabs=kept;
}
if ((PM && PM.adapters && PM.adapters.deepDive && PM.adapters.deepDive.profile === "operating-deep-dive") || I.symbol === "WELCORP") {
    // `indexOf` returns -1 for a section this list does not name, and -1 sorts BEFORE 0 — so
    // `listing` and `verdict`, absent here, led the page ahead of Business on every
    // operating-deep-dive symbol (INDOMIM, measured 2026-08-24). A section the list does not rank
    // must keep its page-model position, not jump to the front: rank it after everything named,
    // and break ties on the model's own order so the result stays stable.
    let order = ['business', 'case', 'peers', 'concall', 'financials', 'execution', 'themes', 'stage', 'ownership', 'risks'];
    let rank = id => { let i = order.indexOf(id); return i === -1 ? order.length : i; };
    let modelPos = new Map(verticalTabs.map((t, i) => [t[0], i]));
    verticalTabs.sort((a, b) => (rank(a[0]) - rank(b[0])) || (modelPos.get(a[0]) - modelPos.get(b[0])));
}
/* RECENT DEVELOPMENTS - the LIST is rendered by the shared M2D library (owner: 'perfect if
   shared'), which the Momentum tab and this page both load. One implementation, two consumers -
   the same rule the rest of M2D follows, and the reason it lives there rather than here: this page
   never loads dashboard.html, and publish.py strips whole tabs, so a renderer owned by the tab
   would vanish from the page.
   This function supplies only the CARD CHROME, because the two consumers frame it differently.
   Renders nothing when the company has none - 293 of 2,653 carry any. */
function developments(){
  var rows = P.developments || [];
  if (!rows.length || !window.M2D || !window.M2D.developments) return '';
  var list = window.M2D.developments(rows);
  if (!list) return '';
  return '<section class=\"dev-block\" aria-label=\"Recent developments\">'
       + '<h2 class=\"dev-head\">Recent developments</h2>' + list + '</section>';
}

function renderVertical(app){let nav='<nav class="tabbar vertical-nav" aria-label="Company research">'+verticalTabs.filter(t=>t[0]!=='themes').map(t=>'<a data-section="'+t[0]+'" href="#'+t[0]+'">'+t[1]+'</a>').join('')+'</nav>';let sections=verticalTabs.map(t=>'<section class="vertical-section" id="'+t[0]+'"><div class="section-head"><p class="eyebrow">Research module</p><h2>'+t[1]+'</h2></div>'+t[2]()+'</section>').join('');app.innerHTML='<div class="company-content">'+hero()+screenInsights()+developments()+'<div class="research-shell">'+nav+sections+'</div></div>'+explorer();initExplorer();let links=[...document.querySelectorAll('.vertical-nav a')],parts=verticalTabs.map(t=>document.getElementById(t[0])).filter(Boolean),setActive=id=>{links.forEach(a=>a.classList.toggle('active',a.dataset.section===id));let active=links.find(a=>a.dataset.section===id);if(active&&matchMedia('(max-width:760px)').matches){let bar=active.parentElement;bar.scrollLeft=active.offsetLeft-(bar.clientWidth-active.offsetWidth)/2}},sync=()=>{let marker=scrollY+135,current=parts[0];parts.forEach(x=>{if(x.offsetTop<=marker)current=x});if(current)setActive(current.id)};links.forEach(a=>a.onclick=()=>setActive(a.dataset.section));addEventListener('scroll',sync,{passive:true});let hash=location.hash.slice(1);if(parts.some(x=>x.id===hash)){let reveal=()=>{document.getElementById(hash).scrollIntoView();setActive(hash)};requestAnimationFrame(reveal);setTimeout(reveal,350);setTimeout(reveal,1800)}else if(verticalTabs.length)setActive(verticalTabs[0][0]);else{/* NO RENDERABLE SECTION IS A VALID STATE, NOT A CRASH. `pageModel.sections` is empty for a company with no stage data and only a shallow offer-document record (ESDS, 2026-08-31), so `verticalTabs[0]` was undefined and this threw 'Cannot read properties of undefined' - killing the render after the hero, so the page showed a title and nothing else with no clue why. Say so instead. */let n=document.querySelector('.company-content')||document.getElementById('store-company');if(n)n.insertAdjacentHTML('beforeend','<div class="empty">Only a preliminary offer-document record exists for this company so far. Detailed sections appear once the document is processed.</div>')};initStage()}
